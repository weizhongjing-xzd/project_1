...
async function createNezhaPoster() {
    ...
    // 加载并绘制角色图片
    try {
        // 优先使用游戏角色图片，如果没有则使用电影图片
        let heroImagePath = path.join(refDir, '哪吒.png');
        if (!fs.existsSync(heroImagePath)) {
            heroImagePath = path.join(refDir, '电影哪吒.png');
        }
        ...
        if (fs.existsSync(heroImagePath)) {
            ...
            // 计算图片尺寸和位置
            const imgWidth = 600;
            const imgHeight = (heroImage.height / heroImage.width) * imgWidth;
            const imgX = centerX - imgWidth / 2;
            const imgY = centerY - imgHeight / 2 + 50; // 稍微下移
            ...
            ctx.save();
            ctx.globalAlpha = 0.2;
            const flameOverlay = ctx.createRadialGradient(centerX, centerY + 100, 0, centerX, centerY + 100, 400);
            ...
            console.log('警告：未找到哪吒角色图片');
        }
    } catch (error) {
        console.log('加载角色图片时出错:', error.message);
    }
...
async function createAoyinPoster() {
    ...
    try {
        // 优先使用游戏角色图片，如果没有则使用电影图片
        let heroImagePath = path.join(refDir, '熬隐.png');
        if (!fs.existsSync(heroImagePath)) {
            heroImagePath = path.join(refDir, '电影熬丙.png');
        }
        ...
        if (fs.existsSync(heroImagePath)) {
            ...
            ctx.save();
            ctx.globalAlpha = 0.2;
            const waterOverlay = ctx.createRadialGradient(centerX, centerY + 100, 0, centerX, centerY + 100, 400);
